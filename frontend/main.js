...
function registerAgentEvents() {
  ...
  agentComm.eventTarget.addEventListener('tool_call', (e) => {
    ...
    agentComm._emitLog(`[事件驱动] 收到智能体指令: ${tool_func} (thread_id: ${thread_id})`, 'tool');
  ...
  agentComm.eventTarget.addEventListener('tool_result', (e) => {
    ...
    agentComm._emitLog('[事件驱动] 工具执行结果: ' + JSON.stringify(result), 'tool');
  });
  // 对话结束
...
agentComm.registerToolEvent('demo_tool', (e) => {
  agentComm._emitLog('[事件驱动] demo_tool 被触发，参数: ' + JSON.stringify(e.detail), 'tool');
});
agentComm.registerToolEvent('demo_tool_result', (e) => {
  agentComm._emitLog('[事件驱动] demo_tool 执行结果: ' + JSON.stringify(e.detail), 'tool');
});
...
function handleSend() {
  ...
  if (!input) return;
  agentComm.sendMessage(input, threadId);
  appendLog('你: ' + input);
  userInput.value = '';
}
...
agentComm.sendToolWithPromise('demo_tool', { thread_id: 'test', param1: '示例参数' }).then(res => {
  agentComm._emitLog('[Promise] demo_tool 执行结果: ' + JSON.stringify(res), 'tool');
});